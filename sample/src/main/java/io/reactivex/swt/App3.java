...
public class App3 {
	...
	private static Observable<String> contentObservable;
	
	private static void createContent() {
		contentObservable = Observable.create( s -> {
			...
			timerObservable.subscribe( time -> { 
				if(count<=0) { s.onComplete(); return; } 
				s.onNext(getTimeStr()); 
			}, e -> { e.printStackTrace(); });
		});
	}
